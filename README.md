...
- **Nombre de la planta = nombre del assembly**. Nombres de los cortes = `{assembly} - CORTE A`
  ...
  El **título en lámina** de la planta es distinto del nombre interno: lleva el sufijo
  ` - FORMAS` vía *Title on Sheet* (00), igual que los cortes muestran `{assembly} - CORTE
  {letra}` en lámina sin que su nombre interno cambie.
